...
public class ZombieController2 : MonoBehaviour {
	...
	private	int lives = LivesCreater.MaxLives;
	...
	void OnTriggerEnter2D( Collider2D other )
	{
		if(other.CompareTag("cat")) {
			...
			other.transform.parent.GetComponent<CatController2>().JoinConga( followTarget, moveSpeed, turnSpeed );
			...
			for( int i = 0; i < 2 && congaLine.Count > 0; i++ )
			{
				...
				cat.parent.GetComponent<CatController2>().ExitConga();
			}
			updateScore();

			int lastId = hearts.Count-1;
			GameObject heart = hearts[lastId];
			hearts.RemoveAt(lastId);
			heart.GetComponent<heartController>().destroyHeart();
